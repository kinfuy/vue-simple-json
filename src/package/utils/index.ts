...
export const mergeArray = (target: Array<any>, scoure: Array<AllowTypeItem>) => {
  const rst = target.map(x => {
    const configType = typeof x === 'string';
    return {
      type: configType ? x : x.type,
      desc: configType ? findAllowType(x, scoure) : x.desc || findAllowType(x.type, scoure)?.desc,
      default: configType ? findAllowType(x, scoure) : x.default || findAllowType(x.type, scoure)?.default,
      slot: configType ? findAllowType(x, scoure) : x.slot || findAllowType(x.type, scoure)?.slot
    };
  });
  return rst;
};
